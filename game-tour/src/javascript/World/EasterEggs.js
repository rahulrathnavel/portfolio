...
import { profile } from '../PortfolioData.js'
        ...
        this.eggs.add = (_options) =>
        {
            ...
            egg.collisionCallback = (_event) =>
            {
                ...
                if(_event.body === this.physics.car.chassis.body && !egg.found)
                {
                    ...
                    {
                        if(window.confirm(`
You found an RR marker!
Small details still count. Want to open Rahul's main portfolio?
                        `))
                        {
                            window.open(profile.portfolio, '_blank', 'noopener')
                        }
