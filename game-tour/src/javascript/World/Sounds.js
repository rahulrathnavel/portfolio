...
export default class Sounds
{
    ...
    setEngine()
    {
        ...
        // Keep the desired values separate from Howler until the file is ready.
        // Calling rate() and volume() on every animation frame while a Howl is
        // loading fills Howler's internal command queue. Howler drains that
        // queue recursively after load, which can overflow the call stack on a
        // slower connection. The engine is visual/gameplay-adjacent audio, so it
        // is safe to begin it only once the audio is ready.
        this.engine.loaded = false
        this.engine.shouldPlay = false
        this.engine.playing = false
        this.engine.targetRate = this.engine.rate.min
        ...
        this.engine.sound = new Howl({
            ...
            onload: () =>
            {
                ...
                this.syncEngineAudio()

                if(this.engine.shouldPlay)
                {
                    this.startEngine()
                }
            },
            ...
            {
                // Keep the driving experience usable when a browser cannot
                // decode an optional sound file. Do not retry every frame.
                console.warn(`Engine audio unavailable: ${_error}`)
            }
        })
        ...
        this.time.on('tick', () =>
        {
            ...
            this.engine.targetRate = this.engine.rate.min + rateAmplitude * this.engine.progress

            // Volume
            const volumeAmplitude = this.engine.volume.max - this.engine.volume.min
            this.engine.targetVolume = (this.engine.volume.min + volumeAmplitude * this.engine.progress) * this.engine.volume.master
    ...
    startEngine()
    {
        this.engine.shouldPlay = true

        // Avoid queueing playback before Howler has finished loading. The
        // onload callback above will call this again when the file is ready.
        if(!this.engine.loaded || this.engine.sound.state() !== 'loaded' || this.engine.sound._playLock)
        {
            return
        }
    ...
    syncEngineAudio()
    {
        // Howler also queues commands while an already-loaded sound is waiting
        // for the browser's playback lock to clear. Treat that state like a
        // load in progress so the frame loop can never rebuild a large queue.
        if(!this.engine.loaded || this.engine.sound.state() !== 'loaded' || this.engine.sound._playLock)
        {
            return
        }

        this.engine.sound.rate(this.engine.targetRate)
        this.engine.sound.volume(this.engine.targetVolume)
    }
    ...
    {
        // The reveal begins only after the player has chosen START, making it
        // the right moment to enable the engine. It still waits for the audio
        // file instead of adding a pre-load command to Howler's queue.
        if(_name === 'reveal')
        {
            this.startEngine()
        }
        ...
        if(item && time > item.lastTime + item.minDelta && (item.velocityMin === 0 || velocity > item.velocityMin))
        {
            ...
            // Sound effects are optional feedback. Skipping one while it is
            // still downloading is preferable to queueing rate/volume/play
            // calls that could later recurse through a very large queue.
            if(sound.state() !== 'loaded' || sound._playLock)
            {
                return
            }
