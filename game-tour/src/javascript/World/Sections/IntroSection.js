...
import { createHeroTexture } from '../../PortfolioData.js'
...
export default class IntroSection
{
    ...
    setTitles()
    {
        const titleTexture = createHeroTexture(['RAHUL', 'RATHNAVEL', 'APPLIED AI / ML ENGINEER'])
        const titleMesh = new THREE.Mesh(
            new THREE.PlaneGeometry(17, 6),
            new THREE.MeshBasicMaterial({ map: titleTexture, transparent: true, depthWrite: false })
        )
        titleMesh.position.set(this.x, this.y + 0.8, 0.08)
        titleMesh.matrixAutoUpdate = false
        titleMesh.updateMatrix()
        this.container.add(titleMesh)

        const proofTexture = createHeroTexture(['RANK 83', 'TECHGIUM FINALIST', 'MERGED PRS / LIVE SYSTEMS'])
        const proofMesh = new THREE.Mesh(
            new THREE.PlaneGeometry(11, 3.9),
            new THREE.MeshBasicMaterial({ map: proofTexture, transparent: true, depthWrite: false })
        )
        proofMesh.position.set(this.x + 12.8, this.y - 3.2, 0.08)
        proofMesh.matrixAutoUpdate = false
        proofMesh.updateMatrix()
        this.container.add(proofMesh)

        // The original physical title pieces remain below for upstream history,
        // but are intentionally not instantiated in Rahul's derived world.
        return
